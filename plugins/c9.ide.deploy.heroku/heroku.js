define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        function install(callback, progress){
            // Check if Heroku is not already installed
            proc.execFile("~/.c9/bin/heroku", {
                args : ["--help"],
                env  : { "GEM_PATH": PATH.join(c9.home, "/.c9/lib/") }
            }, function(err, stdout) {
                if (!err && stdout)
                    return callback();
                    
                progress("Installing Heroku cli (this will take a while)...");
                
                // Install heroku cli
                proc.spawn("gem", {
                    args: ["install", "-n", "~/.c9/bin", "-i", "~/.c9/lib", "heroku"]
                    ...
                    process.stderr.on("data", function(chunk){
                        ...
                        if (chunk.match("Gem::FilePermissionError")) {
                            process.kill();
                            
                            err = new Error("Need sudo rights to install heroku CLI."
                                + " Please type: 'sudo gem install heroku' in "
                                + "a terminal.");
                            err.code = "EPERM";
                            return callback(err);
                        }
                    ...
                    process.stdout.on("data", function(chunk){
                        ...
                        success = success || chunk.match(/Successfully installed heroku/);
                        progress(chunk, true);
                    });

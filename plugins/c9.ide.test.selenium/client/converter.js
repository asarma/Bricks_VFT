/**
 * Converts UI Recordings to Web Driver scripts
 *
...
function UiRecorderToWD(browser){
...
(function(){
    ...
    this.compile = function(actions){
        ...
        for (var i = 0, l = actions.length; i < l; i++) {
            ...
            switch(item.name) {
                case "get":
                    rules.push("browser.getDecoratedTab('" 
                        + item.value.replace(/'/g, "\\'") + "');"); 
                    continue;
                case "setTimeout":
                    rules.push("browser.setWaitTimeout(" + item.value + ")"); 
                    rules.push("browser.setAsyncTimeout(" + item.value + ")");
                    continue;
                case "hold":
                    rules.push("browser.hold(" + item.value + ");");
                    continue;
                case "waitFor":
                    rules.push("browser.waitFor('" 
                        + item.value.replace(/'/g, "\\'") + "')");
                    continue;
                case "execute":
                    rules.push("browser.execute('" 
                        + item.value.replace(/'/g, "\\'") + "')");
                    continue;
                case "executeAsync":
                    rules.push("browser.executeAsync('" 
                        + item.value.replace(/'/g, "\\'") + "')");
                    continue;
            }
            ...
            switch(item.name) {
                case "mousemove":
                    ...
                    break;
                case "mousedown":
                    lastMouseDown = [elId, item.x, item.y, item.offsetX, item.offsetY];
                ...
                case "mouseup":
                    if (lastMouseDown && lastMouseDown[0] == elId) {
                        if (lastMouseDown[1] != item.x || lastMouseDown[2] != item.y) {
                            x = item.x - lastMouseDown[1] + lastMouseDown[3];
                            y = item.y - lastMouseDown[2] + lastMouseDown[4];
                            lastMouseDown = null;
            ...
            function contextToExpression(def, nosel){
                ...
                if(def.htmlXpath)
                    def.xpath = def.htmlXpath;
                    
                if (def.id) 
                    res = def.id;
                else if (def.css)
                    res = "document.querySelector(" + def.css + ")";
                else if (def.eval)
                    res = "(" + def.eval + ")";
                else
                    res = "(typeof apf !== \"undefined\" ? apf.document.selectSingleNode(\"" 
                      + def.xpath
                        .replace(/^html\[1\]\//i, "")
                        .replace(/"/g, "\\\"") + "\") : "
                        + "document.evaluate(\""
                        + def.xpath + "\", document, null, XPathResult.ANY_TYPE, null).iterateNext())";
                    // We first checked if apf was set. If not, use the standard document
...
//                }
            ...
            for (var prop, j = 0; j < item.properties.length; j++) {
                ...
                stack.push("browser.assert('" 
                    + JSON.stringify(prop.element) + "', '" + prop.name + "', '"
...
}).call(UiRecorderToWD.prototype);

module.exports = UiRecorderToWD;

...
/*
    ...
    Ideas:
        ...
        * Multiselect drag&drop
        - sjs really necassary? The file is basically just constructs, no need for doing that in sync
    ...
    Bug:
        ...
        * automatically create the test/selenium directory
        * mnuRunSettings is not defined, error appears very irregular
        - Can't open test with rightclick -> edit
        - Show correct directory in tests listing
        - Save keyboard shortcut gets taken over by the iframe content
        - On save, 'Cannot read property '$editor' of undefined' concorde line 323
        - Any errors will be shown as another action
        - When opening a new stest file, it should first clean the editor before showing it
        - When running a test it'll only show the first two commands as being ran
        - Show the local browser options
...
module.exports = ext.register("ext/selenium/editor", {
    ...
    setDocument : function(doc, actiontracker){
        ...
        if (!doc.hasSeleniumEvents) {
            doc.addEventListener("prop.value", function(e) {
                ...
                dgUiRecorder.setProperty("empty-message", "No test items yet. Click record to add some.");
    ...
    init : function(amlTab) {
        ide.dispatchEvent("init.testrunner");
        
        amlTab.appendChild(mainUiRecorder);
        mainUiRecorder.show();
...
//                return;
    ...
    getProxyUrl : function(){
        var host = (dgUiRecorder.selected
              ? this.findUrl(dgUiRecorder.selected) || tbUiRecordLoc.value
              : tbUiRecordLoc.value || "")
                .replace(/(https?:\/\/[^\/]+)\/.*$/, "$1");
        if(host.indexOf('c9.io') != -1) {
            var url = "/timgl/tests/workspace";
        }
        else {
            var url = "/workspace"; //(?:\/workspace)?)
        }
        return host + url + "/.c9.proxy.html?c9proxyid=" + this.uniqueId;
    },
    ...
    inject : function() {
        ...
        if (useProxy) {
            ...
            if (brSeleniumPreview.src != proxyUrl) {
                (davProject.realWebdav || davProject).writeFile(ide.davPrefix + "/.c9.proxy.html", proxyTemplate, false, function(data, state, extra){
    ...
    loadUrl : function(value, reload) {
        tbUiRecordLoc.setValue(value);
        
        if (this.currentLocation == value && !reload)
            return;
    ...
    reloadFrame : function() {
        this.loadUrl(this.currentLocation, true);
    },
    ...
    start : function(){
        if (!brSeleniumPreview[useProxy ? "proxy-source" : "src"])
            return util.alert("Missing tab",
                "Please load a tab first",
                "Fill in a url to the tab to test in the textbox and press enter");
    ...
    addNewNode : function(tag, name){
        ...
        var node  = dgUiRecorder.selected || dgUiRecorder.xmlRoot;
        var pNode = node.selectSingleNode('ancestor-or-self::' + lut[tag]);
            
        dgUiRecorder.add(el, pNode, node.parentNode == pNode ? node : null);
    },
    
    includeTestFile : function(){
        var requireFile = apf.xmldb.appendChild(dgUiRecorder.xmlRoot, apf.getXml('<test caption="Required File" name="require"/>'), dgUiRecorder.selected.nextSibling);
        dgUiRecorder.select(requireFile);
    },
    ...
    showPropertyMenu : function(e){
        ...
        this.menu = apf.document.documentElement.appendChild(new apf.menu());
        ...
        var pos = ui.getAbsolutePosition(this.iframe);
    ...
    getTests : function(compiled){
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            if (nodes[i].getAttribute("name") != 'require')
                tests[nodes[i].getAttribute("name")] = !compiled ? test : sp.compile(test);
            else
                tests[nodes[i].getAttribute("name")] = nodes[i].getAttribute("value");
        }
    ...
    convertToXml : function(tests) {
        ...
        // Tests
        for (name in tests) {
            if (!name.match(/^test /i) && name != 'require')
                continue;
            ...
            if(name == 'require') {
                testNode.setAttribute("value", test);
                continue;
            }

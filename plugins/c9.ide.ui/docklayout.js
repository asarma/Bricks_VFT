        ...
        */
        ...
        function DockableLayout(developer, deps, options){
            ...
            function resizeTo(widget, width, fromLeft, height, fromTop){
                var delta, shrink, shrinkOther, insertIndex;
                ...
                /*
                    Strategies:
                    - Size is snapped and thus same as a column right now. It 
                        then just changes colspan/rowspan
                    - The size becomes smaller. A new col/row is added at exactly
                        that place in pixels.
                    - The size becomes larger. A col/row is found that matches
                        the size and a new one is added if needed.
                        
                    Missing Strategies:
                    - Growing a fixed row that is making a flex row 
                        smaller (both n and s) [sym]
                    - Growing a flex row to make a fixed row smaller. [sym]
                ...
                */
                ...
                if (delta) {
                    ...
                    insertColumn(width.pixels - total, insertIndex,
                        insertIndex > widget.col + widget.colspan - 1); //!fromLeft
                ...
                if (delta) {
                    ...
                    insertRow(height.pixels - total, insertIndex, 
                        insertIndex > widget.row + widget.rowspan - 1); // && (shrinkOther || shrink) //!fromTop
            ...
            plugin.freezePublicAPI({
                ...
                set columns(v){ 
                    columns = v.split(/\s*,\s*/).map(function(m){
                ...
                set rows(v){ 
                    rows = v.split(/\s*,\s*/).map(function(m){
                        return getRowColEntry(m);
                    });
                    initModel();
                },

...
Terminal.prototype.write = function(data) {
  var rm = this.remoteSize;
  if (rm && (this.cols != rm.x || this.rows != rm.y)) {
    var tmpSize = {x:this.cols, y:this.rows};
    this.resize(rm.x, rm.y);
  }
  this.emit('beforeWrite', data);
  this.writeNoCheckSize(data);
  this.emit('afterWrite', data);
  if (tmpSize) {
    this.resize(tmpSize.x, tmpSize.y);
  }
};
Terminal.prototype.writeNoCheckSize = function(data) {//TODO optimize lines
  var l = data.length
    , i = 0
    , ch;
        ...
        switch (ch) {
          ...
          default:
            ...
            if (ch >= ' ') {
              ...
              if (!this.lines[insertY]) {
                // TODO figure out why does this happen during resizing
                this.log("libterm error attempting to insert into "
                  + insertY + " " + this.lines.length);
                insertY = this.lines.length - 1;
              }
              
              if (!this.insertMode)
                this.lines[insertY][this.x] = [this.curAttr, ch];
              else
                this.lines[insertY].splice(this.x, 0, [this.curAttr, ch]);
...
Terminal.prototype.isLineEmpty = function(line) {
  return !line.some(function(x){ return x[1] && x[1] !== "\xb7" });
};

Terminal.prototype.noScrollBack = function() {
  return (this.applicationKeypad || this.mouseEvents);
};

Terminal.prototype.setSize = function(x, y, local) {
    this.remoteSize = {x:x, y:y};
}
...
Terminal.prototype.resize = function(x, y) {//TODO optimize lines
  ...
  if (rows < y) {
    var toInsert = y - rows;
    var fromScrollBack = noScrollBack ? 0 : Math.min(this.ybase, toInsert);
    this.ybase -= fromScrollBack;
    this.y += fromScrollBack;
    if (toInsert > fromScrollBack) {
      j = toInsert - fromScrollBack;
      while (j--) {
        this.lines.push(this.blankLine());
      }
    }
    ...
    while (toRemove-- > 0) {
      ...
      if (rows - this.y > toRemove + 1 &&
        (noScrollBack || this.isLineEmpty(line))) {
        this.lines.pop();
      } else {
        toRemove++;
        break;
  ...
  var max = this.lines.length - this.rows;
  for (i = 0; i < max; i++) {
...
// Insert P s Column(s) (default = 1) (DECIC), VT420 and up.
// NOTE: xterm doesn't enable this code by default.
Terminal.prototype.insertColumns = function() {//TODO optimize lines
  var param = params[0]

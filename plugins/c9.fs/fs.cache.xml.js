...
define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        model.getChildrenAsync = function(node, cb) {
            fs.readdir(node.path, function(err){
                cb && cb();
            });
        };
        ...
        function load(){
            ...
            function onreaddir(e, orphan) {
                ...
                // update cache
                if (!node) {
                    if (!showHidden && e.path.indexOf("/.") !== -1)
                        return;
                    // TODO might need to revert to the old way of keeping orphans
                    if (!e.result[1].length) 
                        return;
                ...
                e.result[1].forEach(function(stat){
                    if (!showHidden && stat.name.charAt(0) == ".")
                        return;
                    var name = stat.name;
                    ...
                    createNode(path, stat, existing[name]);
                });
                ...
                if (!orphan)
                    emit("readdir", {path : e.path, parent : node});
            ...
            function onstat(e) {
                if (!e.error) {
                    ...
                    if (there != !!e.result[1]) {
                        ...
                        createNode(e.path, e.result[1], node);
                    }
                }
            }
            ...
            fs.on("afterReadFile", function(e){
                return onstat({
                    path   : e.path, 
                    result : [0, e.result[0] && e.result[0].code == "ENOENT" 
                        ? false : true]
                });
            }, plugin);
        ...
        function createNode(path, stat, updateNode) {
            if (orphans[path]) {
            ...
            if (!modified.length)
                modified.push(parent);
            var isOpen = modified[0].isOpen;
            if (isOpen) model.close(modified[0], undefined, true);
            modified.forEach(function(n) {
                n.children = null;
            });
            if (isOpen) model.open(modified[0]);
        ...
        function deleteNode(node, silent) {
            ...
            var wasOpen = parent.isOpen;
            if (wasOpen) {
                model.close(parent, undefined, true);
            }

    ...
    function resolvePath(path, options, callback) {
        ...
        function check(err, path) {
            ...
            if (!options.nocheck) {
                ...
                if (!(path === base || path.substr(0, localRoot.length) === localRoot)) {
                    err = new Error("EACCESS: '" + path + "' not in '" + localRoot + "'");
                    err.code = "EACCESS";
                    return callback(err);
                }
            }
...
////////////////////////////////////////////////////////////////////////////////
    ...
    function readfile(path, options, callback) {
        ...
        open(path, "r", umask & 0666, options, function (err, path, fd, stat) {
            if (err) return callback(err);
            if (stat.isDirectory()) {
                fs.close(fd);
    ...
    function WatcherWrapper(path, options, callback) {
        ...
        function watch(callback) {
            ...
            try {
                if (options.file) {
                    watcher = fs.watchFile(path, { persistent: false }, buffer);
                    watcher.close = function () { fs.unwatchFile(path); };
                }
                else {
                    watcher = fs.watch(path, { persistent: false }, buffer);
                }
            } catch(e) {
                return sendError(e);
            }
        ...
        // Buffering helps mostly for directory events
        function buffer(event, filename) {
            // No need to buffer if we can't expect more events
            if (persistent === false)
                return listen(event, filename);
            
            // Wait for 30ms to see if a new event comes in
            clearTimeout(timer);
            timer = setTimeout(function() {
                listen(event, filename);
            }, 30);
        ...
        function sendToAllListeners(event, filename, entry, files){
